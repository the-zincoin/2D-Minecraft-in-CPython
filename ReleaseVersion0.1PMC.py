import pygame,json,math,os,ast
...
resolutionInTile = int(10 if loaded_data['resolutionInTile(InPixel)'] < 10 else loaded_data['resolutionInTile(InPixel)']) #(fixed at 10 and higher)
...
def calyBorder():
    verticalNum = int(math.ceil(height/resolutionInTile))+10 #adds 3 to prevent black areas when displaying
    ...
    offsetTilesBy = 0-((blockDisplayheight-height) /2)#offset from y0 that helps the tiles fit cleanly
...
with open('PMDataFiles/BlockLabelFull.json', 'r') as file:
    blockFileNames = list(json.load(file))
...
#load all Minecraft block textures
for i in range(len(blockFileNames)):
    if ".mcmeta" not in blockFileNames[i]:
        blockLoaded = pygame.image.load(f'ImageFiles\{blockFileNames[i]}').convert_alpha()
        blockLoaded = pygame.transform.scale(blockLoaded,(resolutionInTile,resolutionInTile))
        blockAtlas.append(blockLoaded)
        newblockNames.append(blockFileNames[i])


#log textures in PMDataFiles
if os.path.exists('PMDataFiles\BlockImagesFull.json'):
    pass
else:
    with open('PMDataFiles\BlockImagesFull.json', 'w') as file:
        json.dump(newblockNames,file,indent=4)

#load special textures
Sky = pygame.image.load('ImageFiles\Sky16x16.png')
Sky2 = pygame.transform.scale(Sky,(resolutionInTile,resolutionInTile))


Void = pygame.image.load('ImageFiles/Void.jpg')
Void2 = pygame.transform.scale(Void,(resolutionInTile,resolutionInTile))
...
def convertIntoBlocks(layerCurrent,offset):
    chunk = [] #convert height int in iterable layer current into blittable surfaces for displaying
    ...
    print("CLD",len(currentLayerData))
    #print(len(currentLayerData))
    chunk.append(list(currentLayerData))
    for n,height in enumerate(layerCurrent):
        currentLayerData = []
        currentLayerData = [Sky2 for i in range(256-int(height))] #add Sky texture data
        dirtHeight,deepSlateHeight = pRVdirH[n],pRVdepH[n] #number of blocks of dirt and deepslate respectively
        currentLayerData.append(blockAtlas[403]) #add grass block data
        for i in range(dirtHeight):
            currentLayerData.append(blockAtlas[336])  #add dirt block data
        for i in range(int(height) - 1 - dirtHeight - (deepSlateHeight)):
            currentLayerData.append(blockAtlas[845]) #add stone block data
        for i in range(deepSlateHeight):
            currentLayerData.append(blockAtlas[309]) #add deepslate data
        #print(len(currentLayerData))
        chunk.append(list(currentLayerData))
    return list(chunk) 
    ...
    for i,chunk in enumerate(worldData.currentRenderedChunks):
        for x in range(16):
            for y in range(0,worldData.verticalNum):
                block = Sky2 if blockYReference[y][0] > 255 else Void2 #ensures that tiles beyond 256(world height) and below 0(bedrock) is Sky and Void respectively
                block = (blockAtlas[48] if blockYReference[y][0] == 0 else chunk[1][x][255-blockYReference[y][0]]) if blockYReference[y][0] >= 0 else Void2 #overrides any tile at y=0 into bedrock
                worldData.chunkSurfaces[i].blit(block,(x*resolutionInTile,blockYReference[y][1])) #blit tiles on chunk Surface
...
while True:
    ...
    if keyPressed[pygame.K_LEFT]:
        ...
        for index,value in enumerate(worldData.currentRenderedChunks):
            currChunk = value[2][0]
            ...
            worldData.currentRenderedChunks[index][2][0] += xOffSet #change chunks by velocity and whether chunk needs to be wrapped
            xOffSet  = 0
    ...
    if keyPressed[pygame.K_UP]:
        ...
        for index,value in enumerate(blockYReference):
            currTile = value[1]
            ...
            if currTile < worldData.bottomWrap:
                ...
                yOffSet = 0-(currTile-worldData.topWrap) - (worldData.bottomWrap-currTile)  + velocity
            ...
            blockYReference[index][1] += yOffSet
            blockYReference[index][0] += tileLOffset
            yOffSet = 0
    ...
    if keyPressed[pygame.K_DOWN]:
        ...
        for index,value in enumerate(blockYReference):
            ...
            if currTile > worldData.topWrap:
                ...
                yOffSet = (worldData.bottomWrap - currTile) +(currTile-worldData.topWrap) - velocity
                tileLOffset = -worldData.verticalNum
            blockYReference[index][1] += yOffSet
            blockYReference[index][0] += tileLOffset #changes the index of rendering as the new tile displayed is at a different position after being wrapped around (+ as you move to after previous index 255)
            yOffSet = 0
