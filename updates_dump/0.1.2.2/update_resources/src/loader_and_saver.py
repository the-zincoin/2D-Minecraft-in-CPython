...
from data_file_manager import loadFile,dumpFile
...
def dumpWorldData(path,gameData,currPlayerPos,seed,resources,cache_size):
    data = {}
    data['currentPlayerPos'] = currPlayerPos
    data['render_distance'] = resources.interactive_data["settings"]["render_distance"]
    data['seed'] = seed
...
def dumpData(gameData, path, currPlayerPos, seed,newChunks,resources,cache_size):
    ...
    dumpWorldData(nPath,gameData,currPlayerPos,seed,resources,cache_size)
...
def saveWorld(worldName,seed,gameData,currPlayerPos,oldData,resources,cache_size,cache_max_size):
    if not os.path.exists(f"minecraft/worlds/{worldName}"):
        os.makedirs(f"minecraft/worlds/{worldName}")
        os.makedirs(f"minecraft/worlds/{worldName}/chunks")
        os.makedirs(f"minecraft/worlds/{worldName}/gameSaveData")
    ...
    initFolders(newChunks,f"minecraft/worlds/{worldName}/chunks")
    dumpData(gameData,f"minecraft/worlds/{worldName}/chunks",currPlayerPos,seed,newChunks,resources,cache_full_size)
...
def loadWorld(worldName):
    path = f"minecraft/worlds/{worldName}"
