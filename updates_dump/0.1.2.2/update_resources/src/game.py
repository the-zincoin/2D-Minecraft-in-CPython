...
def renderChunks(currentPlayerX,currentPlayerY,screen,gameData,resources):
    screen.fill((128,188,252))
    renderDistance = resources.interactive_data["settings"]["render_distance"]
...
def run(screen,gameData,pressed,currentPlayerX,currentPlayerY,key,resources,perlinConfig):
    from player import renderPlayer
    import chunk_processor as p
    data = p.playerMovement(key,gameData,pressed,currentPlayerX,currentPlayerY,resources,perlinConfig)
    coord,pressed = (data[0],data[1]),data[2]
    currentPlayerX,currentPlayerY = coord[0],coord[1]
    if pressed:
        renderChunks(currentPlayerX,currentPlayerY,screen,gameData,resources)   
        renderPlayer(screen,gameData.tileRes,resources)
        pressed = False
