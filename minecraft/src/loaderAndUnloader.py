import os
import pygame,math
from processingClasses import Chunk
from fileManager import loadFile,dumpFile
...
def saveWorld(worldName,seed,gameData,currPlayerPos,oldData,config):
    ...
    dumpData(gameData,f"worlds/{worldName}/chunks",currPlayerPos,seed,newChunks,config)
...
def loadWorld(worldName):
    ...
    if os.path.exists(path):
        surfaceMetaData = loadFile(f"{path}/gameSaveData/surfaceMetadata.json","r")
        surfacesForChunks = loadChunkAttributes(loadFile(f"{path}/gameSaveData/chunkAttributes.json","r"),f"{path}/chunks",surfaceMetaData)
        worldData = loadFile(f"{path}/gameSaveData/gameStateData.json","r")
