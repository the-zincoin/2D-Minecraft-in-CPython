import random
def initSplash():
    with open("assets/config/hardcoded/menus/splashes.txt","r") as file:
        splashList = list(file.readlines())
        return random.choice(splashList)
def renderSplash(menu_screen_surface,fontName,splash,pulseSpeed,sizeRange,timePassed,config):
    import math,pygame
