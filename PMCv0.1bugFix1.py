...
xOffSet = 0
yOffSet = 0
seed = random.randint(0,2**32) if int(loaded_data['seed']) == 0 else int(loaded_data['seed']) #seed for perlinNoise @Perlin 2d #now random seed when set to 0
...
lengthSide = 32
#getTile from atlas through fileName
...
#check if file already loaded from atlas
...
#caption for window
pygame.display.set_caption('Version0.1Bugfix1 Paper Minecraft Python')
...
def convertIntoBlocks(layerCurrent,offset):
    ...
    for n,height in enumerate(layerCurrent):
        ...
        for y in range(int(height)):
            if y == 0 and (y + 256 - height) < 128: 
                currentLayerData.append(checkIfTileLoaded("grass_block_side.png")) #add grass block data
            elif y <= dirtHeight and (y + 256 - height) < 128: 
                currentLayerData.append(checkIfTileLoaded("dirt.png"))
            elif (y + 256 - height) < (192 + deepSlateHeight):
                currentLayerData.append(checkIfTileLoaded("stone.png"))
            else:
                currentLayerData.append(checkIfTileLoaded("deepslate.png"))
