...
def updateChunks(config,gameData,playerX,playerY):
...
def playerMovement(key,gameData,pressed,currentPlayerX,currentPlayerY,config):
    import pygame
    velocity = gameData.velocity
    if key[pygame.K_RIGHT]:
        currentPlayerX -= velocity
        pressed = True
    elif key[pygame.K_LEFT]:
        currentPlayerX += velocity
        pressed = True
    elif key[pygame.K_UP]:
        currentPlayerY += velocity
        pressed = True
    ...
    if pressed:
        updateChunks(config,gameData,currentPlayerX, currentPlayerY)
